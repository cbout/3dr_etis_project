...
\subsection*{Save video stream}
\label{sec:savestream}

\paragraph{} In order to reuse or interpret video stream, we give the posibility to save it in three forms.


\paragraph{Video file} First solution, register it in a video file (.avi in this case). To be done, we use again the library gstreamer where we redirect the input flow (udpsrc) to a file with the following sequence : "! queue ! avimux ! filesink location=capture.avi". Thus the string to put in gst\_parse\_launch become : 
...
\paragraph{Pictures files} Second possibility which completed the first one, save each frame of the video stream on pictures. To do this, we redirect again the input flow (udpsrc) with gstreamer but in this case in multiple files as shown this code : "! queue ! videorate ! jpegenc ! multifilesink location="frame/jpg/frame\%06d.jpg"". Note that in this example we write frames on .jpg files but we can change the format to .bmp for example replacing "jpegenc" by "avenc\_bmp". Thus the final string to put in gst\_parse\_launch become :
...
\paragraph{Memory buffer} Last possibility which replace the second one, register each frames of the stream in a memory during the program execution. To do this, we redirect again the input flow (udpsrc) with gstreamer but in this case in a char table. To do this we have first to change the string to parse adding the following fileds : "! queue ! appsink name=sink emit-signals=true sync=false max-buffers=1 drop=true". \\
\noindent Then we have to get the sink before playing the pipeline. After that we do the following instructions :

\begin{enumerate}
\item Get the sample and the buffer
\item Get the capture, the structure, and so the width and the height
\item Get the map and the image matrix
\end{enumerate}

\noindent The code of previous instructions is :
...
\item To register video stream in a file with gstreamer : https://www.raspberrypi.org/forums/viewtopic.php?t=65187
\item To register video stream frames in files with gstreamer : http://www.tal.org/tutorials/timelapse-video-gstreamer
\end{itemize}
